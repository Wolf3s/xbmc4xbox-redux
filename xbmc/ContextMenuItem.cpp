...
bool CContextMenuItem::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  ...
  if (addon->ExtraInfo().find("reuselanguageinvoker") != addon->ExtraInfo().end())
  {
    ...
    reuseLanguageInvoker = addon->ExtraInfo().find("reuselanguageinvoker")->second == "true" && appXbox->HasMemoryUpgrade();
  }
...
{
  if (IsGroup())
    return StringUtils::Format("CContextMenuItem[group, id=%s, parent=%s, addon=%s]", m_groupId.c_str(),
                               m_parent.c_str(), m_addonId.c_str());
  else
    return StringUtils::Format("CContextMenuItem[item, parent=%s, library=%s, addon=%s]", m_parent.c_str(),
                               m_library.c_str(), m_addonId.c_str());
}
