                                         ...
                                         const ScraperPtr& info, int episode)
{
  ...
  if (m_type == ADDON::AddonType::SCRAPER_ALBUMS)
  {
    CAlbum album;
    bNfo = GetDetails(album);
  }
  else if (m_type == ADDON::AddonType::SCRAPER_ARTISTS)
  {
    CArtist artist;
    bNfo = GetDetails(artist);
  }
  else if (m_type == ADDON::AddonType::SCRAPER_TVSHOWS || m_type == ADDON::AddonType::SCRAPER_MOVIES ||
           m_type == ADDON::AddonType::SCRAPER_MUSICVIDEOS)
  {
    // first check if it's an XML file with the info we need
    CVideoInfoTag details;
    bNfo = GetDetails(details);
    if (episode > -1 && bNfo && m_type == ADDON::AddonType::SCRAPER_TVSHOWS)
    {
      int infos=0;
...
int CNfoFile::Load(const std::string& strFile)
{
  ...
  {
    m_doc.assign(reinterpret_cast<char*>(&buf[0]), buf.size());
    m_headPos = 0;
    return 0;
  }
  m_doc.clear();
...
std::vector<ScraperPtr> CNfoFile::GetScrapers(ADDON::AddonType::Type type, const ScraperPtr& selectedScraper)
{
  AddonPtr addon;
