...
void CApplicationStackHelper::OnPlayBackStarted(const CFileItem& item)
{
  ...
  {
    boost::shared_ptr<const CFileItem> stack = GetRegisteredStack(item);
...
bool CApplicationStackHelper::InitializeStack(const CFileItem & item)
{
  ...
  boost::shared_ptr<CFileItem> stack = boost::make_shared<CFileItem>(item);
...
boost::optional<int> CApplicationStackHelper::InitializeStackStartPartAndOffset(const CFileItem& item)
{
  CVideoDatabase dbs;
  ...
  if (m_currentStackIsDiscImageStack)
  {
    ...
    if (startoffset == STARTOFFSET_RESUME) // selected file is not specified, pick the 'last' resume point
    {
      if (dbs.Open())
      {
        ...
        CLog::Log(LOGERROR, "Cannot open VideoDatabase");
    }

    // make sure that the selected part is within the boundaries
    if (selectedFile <= 0)
    {
      CLog::Log(LOGWARNING, "Selected part %i out of range, playing part 1", selectedFile);
      selectedFile = 1;
    }
    else if (selectedFile > m_currentStack->Size())
    {
      CLog::Log(LOGWARNING, "Selected part %i out of range, playing part %i", selectedFile,
                 m_currentStack->Size());
...
boost::shared_ptr<const CFileItem> CApplicationStackHelper::GetRegisteredStack(
...
{
  const CApplicationStackHelper::Stackmap::const_iterator it = m_stackmap.find(item.GetPath());
  return it != m_stackmap.end() && it->second != NULL;
}

void CApplicationStackHelper::SetRegisteredStack(const CFileItem& item,
                                                 boost::shared_ptr<CFileItem> stackItem)
{
  GetStackPartInformation(item.GetPath())->m_pStack = boost::move(stackItem);
}
...
{
  const CApplicationStackHelper::Stackmap::const_iterator it = m_stackmap.find(key);
  if (it == m_stackmap.end())
    return boost::make_shared<StackPartInformation>();
  return it->second;
}
