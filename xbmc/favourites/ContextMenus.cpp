...
bool isSamePath(const CFileItemPtr& favourite, const CFileItemPtr &item) { return favourite->GetPath() == item->GetPath(); }

bool CFavouriteContextMenuAction::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  CFileItemList items;
  CServiceBroker::GetFavouritesService().GetAll(items);

  const VECFILEITEMS::const_iterator it = std::find_if(items.cbegin(), items.cend(), boost::bind(&isSamePath, _1, item));
...
bool CMoveUpFavourite::DoExecute(CFileItemList& items, const boost::shared_ptr<CFileItem>& item) const
{
...
bool CMoveDownFavourite::DoExecute(CFileItemList& items,
                                   const boost::shared_ptr<CFileItem>& item) const
{
  return FAVOURITES_UTILS::MoveItem(items, item, +1);
}
...
bool CRemoveFavourite::DoExecute(CFileItemList& items, const boost::shared_ptr<CFileItem>& item) const
{
  return FAVOURITES_UTILS::RemoveItem(items, item);
}

bool CRenameFavourite::DoExecute(CFileItemList&, const boost::shared_ptr<CFileItem>& item) const
{
...
namespace
{
boost::shared_ptr<CFileItem> ResolveFavouriteItem(const CFileItem& item)
{
  boost::shared_ptr<CFileItem> targetItem =
      CServiceBroker::GetFavouritesService().ResolveFavourite(item);
  if (targetItem)
    targetItem->SetProperty("hide_add_remove_favourite", true);
...
bool CFavouritesTargetBrowse::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  return FAVOURITES_UTILS::ExecuteAction(CFavouritesURL(*item, -1));
}

std::string CFavouritesTargetResume::GetLabel(const CFileItem& item) const
{
  const boost::shared_ptr<CFileItem> targetItem = ResolveFavouriteItem(item);
  if (targetItem)
...
bool CFavouritesTargetResume::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  const boost::shared_ptr<CFileItem> targetItem = ResolveFavouriteItem(*item);
  if (targetItem)
    return FAVOURITES_UTILS::ExecuteAction(CExecString("PlayMedia", *targetItem, "resume"));
...
bool CFavouritesTargetPlay::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  const boost::shared_ptr<CFileItem> targetItem = ResolveFavouriteItem(*item);
  if (targetItem)
    return FAVOURITES_UTILS::ExecuteAction(CExecString("PlayMedia", *targetItem, "noresume"));
...
bool CFavouritesTargetInfo::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  const boost::shared_ptr<CFileItem> targetItem = ResolveFavouriteItem(*item);
  if (targetItem)
...
bool CFavouritesTargetContextMenu::Execute(const boost::shared_ptr<CFileItem>& item) const
{
  const boost::shared_ptr<CFileItem> targetItem = ResolveFavouriteItem(*item);
  if (targetItem)
  {
