...
CFavouritesURL::CFavouritesURL(const std::string& favouritesURL) : m_valid(false), m_action(Action::UNKNOWN), m_windowId(-1), m_isDir(false)
{
  const CURL url(favouritesURL);
  if (url.GetProtocol() != "favourites")
  {
    CLog::Log(LOGERROR, "Invalid protocol");
    return;
  }
...
CFavouritesURL::CFavouritesURL(const CExecString& execString) : m_exec(execString), m_valid(false), m_action(Action::UNKNOWN), m_windowId(-1), m_isDir(false)
{
  if (m_exec.IsValid())
    m_valid = Parse(GetActionId(m_exec.GetFunction()), m_exec.GetParams());
}
...
CFavouritesURL::CFavouritesURL(Action action, const std::vector<std::string>& params)
  : m_exec(GetActionString(action), params), m_valid(false), m_action(Action::UNKNOWN), m_windowId(-1), m_isDir(false)
{
  if (m_exec.IsValid())
...
CFavouritesURL::CFavouritesURL(const CFileItem& item, int contextWindow)
  : m_exec(item, std::to_string(contextWindow)), m_valid(false), m_action(Action::UNKNOWN), m_windowId(-1), m_isDir(false)
{
  if (m_exec.IsValid())
    m_valid = Parse(GetActionId(m_exec.GetFunction()), m_exec.GetParams());
}
...
bool CFavouritesURL::Parse(CFavouritesURL::Action action, const std::vector<std::string>& params)
{
  ...
  switch (action)
  {
    case Action::ACTIVATE_WINDOW:
      if (params.empty())
      {
        CLog::Log(LOGERROR, "Missing parameter");
        return false;
      }
      ...
      m_actionLabel =
          StringUtils::Format(g_localizeStrings.Get(15220).c_str(), // Show content in '<windowname>'
                              g_localizeStrings.Get(m_windowId).c_str());
      break;
  ...
  m_path = StringUtils::Format("favourites://%s", CURL::Encode(GetExecString()).c_str());
