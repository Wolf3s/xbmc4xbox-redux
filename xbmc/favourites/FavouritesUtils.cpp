...
bool ChooseAndSetNewName(CFileItem& item)
{
  ...
  if (CGUIKeyboardFactory::ShowAndGetInput(label, g_localizeStrings.Get(16008),
...
bool ChooseAndSetNewThumbnail(CFileItem& item)
{
  ...
  {
    const boost::shared_ptr<CFileItem> current = boost::make_shared<CFileItem>("thumb://Current", false);
  ...
  const boost::shared_ptr<CFileItem> none = boost::make_shared<CFileItem>("thumb://None", false);
...
bool MoveItem(CFileItemList& items, const boost::shared_ptr<CFileItem>& item, int amount)
{
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (items[i]->GetPath() == items[i]->GetPath())
      break;
  }
...
bool RemoveItem(CFileItemList& items, const boost::shared_ptr<CFileItem>& item)
{
...
bool ShouldEnableMoveItems()
{
  ...
  CGUIWindowManager &mgr = CServiceBroker::GetGUI()->GetWindowManager();
