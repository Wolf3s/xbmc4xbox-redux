...
protected:
  virtual bool OnPlayPartSelected(unsigned int part)
  {
    ...
    FAVOURITES_UTILS::ExecuteAction(CExecString
        ("PlayMedia", *m_item, StringUtils::Format("playoffset=%u", part - 1)));
    return true;
  }

  virtual bool OnResumeSelected()
  {
    FAVOURITES_UTILS::ExecuteAction(CExecString("PlayMedia", *m_item, "resume"));
    return true;
  }

  virtual bool OnPlaySelected()
  {
    FAVOURITES_UTILS::ExecuteAction(CExecString("PlayMedia", *m_item, "noresume"));
    return true;
  }

  virtual bool OnQueueSelected()
  {
    FAVOURITES_UTILS::ExecuteAction(CExecString("QueueMedia", *m_item, ""));
    return true;
  }
...
bool CGUIWindowFavourites::OnSelect(int itemIdx)
{
  ...
  const CFileItemPtr item = (*m_vecItems)[itemIdx];
  ...
  const boost::shared_ptr<CFileItem> target = CServiceBroker::GetFavouritesService().ResolveFavourite(*item);
  if (!target)
    return false;
  ...
  if (targetItem.HasVideoInfoTag() && (!targetItem.m_bIsFolder || isPlayMedia))
  {
    ...
    CVideoSelectActionProcessor proc(boost::make_shared<CFileItem>(targetItem));
...
bool CGUIWindowFavourites::OnAction(const CAction& action)
{
  ...
  {
    const boost::shared_ptr<CFileItem> target =
        CServiceBroker::GetFavouritesService().ResolveFavourite(*(*m_vecItems)[selectedItem]);
    if (!target)
      return false;

    const boost::shared_ptr<CFileItem> item = boost::make_shared<CFileItem>(*target);
    ...
    if (CPlayerUtils::IsItemPlayable(*item))
    {
      ...
      if (target.GetAction() != CFavouritesURL::Action::PLAY_MEDIA)
      {
        ...
        target = CFavouritesURL(CFavouritesURL::Action::PLAY_MEDIA,
                                std::vector<std::string>(1, StringUtils::Paramify(item->GetPath())));
      }
  ...
  {
    const boost::shared_ptr<CFileItem> targetItem =
        CServiceBroker::GetFavouritesService().ResolveFavourite(*(*m_vecItems)[selectedItem]);
