...
bool CDirectory::Create(const CURL& url)
{
  try
  {
    ...
    boost::movelib::unique_ptr<IDirectory> pDirectory(CDirectoryFactory::Create(realURL));
    if (pDirectory)
                                       ...
                                       bool expandImages)
{
  for (int i=0; i< items.Size(); ++i)
  {
    ...
    {
      boost::movelib::unique_ptr<IFileDirectory> pDirectory(CFileDirectoryFactory::Create(pItem->GetURL(),pItem.get(),mask));
      if (pDirectory)
