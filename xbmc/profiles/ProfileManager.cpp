...
void CProfileManager::Initialize(const boost::shared_ptr<CSettings>& settings)
{
  m_settings = settings;
...
bool CProfileManager::Load()
{
  ...
  if (CFile::Exists(file))
  {
    ...
    {
      CLog::Log(LOGERROR, "CProfileManager: error loading %s, Line %i\n%s", file.c_str(),
...
bool CProfileManager::LoadProfile(unsigned int index)
{
  ...
  if (g_SkinInfo != NULL && !m_previousProfileLoadedForLogin)
  ...
  if (!settings->Load())
  {
    CLog::Log(LOGFATAL, "CProfileManager: unable to load settings for profile \"%s\"",
...
void CProfileManager::FinalizeLoadProfile()
{
  ...
  {
    CLog::Log(LOGFATAL, "Unable to load language for profile \"%s\"",
...
bool CProfileManager::DeleteProfile(unsigned int index)
{
  ...
  dlgYesNo->SetLine(0, StringUtils::Format(str.c_str(), profile->getName().c_str()));
  dlgYesNo->SetLine(1, "");
  dlgYesNo->SetLine(2, "");
  dlgYesNo->Open();
  ...
  CFileItemPtr item =
      boost::make_shared<CFileItem>(URIUtils::AddFileToFolder(GetUserDataFolder(), strDirectory));
...
const CProfile& CProfileManager::GetCurrentProfile() const
{
  ...
  CLog::Log(LOGERROR, "CProfileManager: current profile index (%u) is outside of the valid range (%" PRIdS ")", m_currentProfile, m_profiles.size());
  return EmptyProfile;
}
