    ...
    if (name.name_id == TT_NAME_ID_FONT_FAMILY)
    {
          ...
          name.platform_id == TT_PLATFORM_APPLE_UNICODE)
      {
        ...
        if (CCharsetConverter::utf16BEtoUTF8(nameEnc, familyName))
          ...
          CLog::Log(LOGERROR, "Failed to convert the font name string encoded as \"UTF-16BE\"");
      }
      else if (name.platform_id == TT_PLATFORM_MACINTOSH && familyName.empty())
      {
        ...
        if (!CCharsetConverter::MacintoshToUTF8(nameEnc, familyName))
          CLog::Log(LOGERROR, "Failed to convert the font name string encoded as \"macintosh\"");
      }
      else
      {
                                     ...
                                     std::set<std::string>& familyNames)
{
  ...
  do
  {
    ...
    if (familyName.empty())
    {
      ...
      if (familyName.empty())
      {
        CLog::Log(LOGERROR, "Family name missing in the font");
        return false;
      }
    }
...
std::string UTILS::FONT::GetFontFamily(std::vector<uint8_t>& buffer)
{
  ...
  {
    CLog::Log(LOGERROR, "Failed to process font memory buffer");
  }
