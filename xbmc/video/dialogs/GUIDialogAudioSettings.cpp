...
void CGUIDialogAudioSettings::OnSettingChanged(const boost::shared_ptr<const CSetting>& setting)
{
  ...
  {
    m_audioStream = boost::static_pointer_cast<const CSettingInt>(setting)->GetValue();
  ...
  {
    m_passthrough = boost::static_pointer_cast<const CSettingBool>(setting)->GetValue();
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetBool(CSettings::SETTING_AUDIOOUTPUT_PASSTHROUGH, m_passthrough);
  }
}

void CGUIDialogAudioSettings::OnSettingAction(const boost::shared_ptr<const CSetting>& setting)
{
  if (setting == NULL)
    return;
...
void CGUIDialogAudioSettings::InitializeSettings()
{
  ...
  dependencyAudioOutputPassthroughDisabled.Or()
      ->Add(boost::make_shared<CSettingDependencyCondition>(SETTING_AUDIO_PASSTHROUGH, "true",
  ...
  AddSlider(groupAudio, SETTING_AUDIO_VOLUME_AMPLIFICATION, 660, SettingLevel::Basic, videoSettings.m_VolumeAmplification, 14054, VOLUME_DRC_MINIMUM * 0.01f, (VOLUME_DRC_MAXIMUM - VOLUME_DRC_MINIMUM) / 6000.0f, VOLUME_DRC_MAXIMUM * 0.01f);

  // audio delay setting
  boost::shared_ptr<CSettingNumber> settingAudioDelay = AddSlider(
      ...
      usePopup);
  boost::static_pointer_cast<CSettingControlSlider>(settingAudioDelay->GetControl())->SetFormatter(SettingFormatterDelay);

  // audio stream setting
  AddAudioStreams(groupAudio, SETTING_AUDIO_STREAM);

  // audio output to all speakers setting
  boost::shared_ptr<CSettingBool> settingOutputToAllSpeakers = AddToggle(groupAudio, SETTING_AUDIO_OUTPUT_TO_ALL_SPEAKERS, 252, SettingLevel::Basic, videoSettings.m_OutputToAllSpeakers);
...
void CGUIDialogAudioSettings::AddAudioStreams(const boost::shared_ptr<CSettingGroup>& group,
                                                       ...
                                                       void* data)
{
  ...
  for (int i = 0; i < audioStreamCount; ++i)
  {
    ...
    strItem = StringUtils::Format(strFormat.c_str(), strLanguage.c_str(), info.name.c_str(), info.channels);
